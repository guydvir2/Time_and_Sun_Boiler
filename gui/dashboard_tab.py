"""
Dashboard Tab — landing page inside the Control tab notebook.
Shows live status, today's plan/prediction, and quick actions.
...
# ── Palette (shared with settings_tab / control_tab) ─────────
ACC_BLUE   = "#3b82f6"
ACC_GREEN  = "#22c55e"
ACC_RED    = "#ef4444"
ACC_ORANGE = "#f59e0b"
...
TEXT_FG    = "#e2e8f0"
TEXT_DIM   = "#94a3b8"
TEXT_VAL   = "#f1f5f9"    # bright white for values
...
class DashboardTab:
    ...
    def __init__(self, parent, config, runtime_settings, scheduler,
        ...
        self._prediction     = None   # cached calc dict
    ...
    def _card(self, parent, title="", pady=(0, 8)):
        ...
        content = tk.Frame(inner, bg=CARD_BG, padx=12, pady=10)
        content.pack(fill="both", expand=True)
        return content

    def _card_in(self, parent, title="", expand=False):
        """Card that fills a grid cell. expand=True makes it fill vertically."""
        outer = tk.Frame(parent, bg=ACC_BLUE, pady=1)
    ...
    def _kv(self, parent, key, value="—", value_color=TEXT_VAL):
        """One key-value row. Returns the value Label so it can be updated."""
        r = self._row(parent)
    ...
    def _create_ui(self):
        ...
        cols.columnconfigure(0, weight=3)   # Today's Plan — wider
        cols.columnconfigure(1, weight=2)   # Weekly
        cols.columnconfigure(2, weight=2)   # Actions
        ...
        # Right column: Start Now fixed, One-Shot expands
        right.rowconfigure(0, weight=0)
        right.rowconfigure(1, weight=1)
        right.columnconfigure(0, weight=1)

        self._build_today_card(left)
    ...
    def _build_today_card(self, parent):
        # Use a pack-based outer that expands to fill the left column
        outer = tk.Frame(parent, bg=ACC_BLUE, pady=1)
        outer.pack(fill="both", expand=True)
        inner = tk.Frame(outer, bg=CARD_BG)
        inner.pack(fill="both", expand=True)
        tk.Label(inner, text="TODAY'S PLAN", bg=CARD_BG, fg=TEXT_DIM,
        ...
        tk.Label(box, text="WEATHER PREDICTION",
        ...
        self._pred_status_lbl  = tk.Label(
            box, text="", bg=CARD_BG, fg=TEXT_DIM,
    ...
    def _refresh_weekly(self):
        """Called when MQTT weekly cmd updates a preset — refresh widget vars."""
        DAYS = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"]
        for w in self._weekly_widgets:
            pid = w["id"]
    ...
    def _build_start_now_card(self, parent):
        ...
        tk.Label(box, text=f"via  {mode}",
                 bg=CARD_BG, fg=TEXT_DIM,
                 font=("Segoe UI", 8)).pack(anchor="w", pady=(0, 6))

        self._big_btn(box, "⚡  Start Now",
                      self._start_now, bg=ACC_GREEN
                      ).pack(fill="x")
    ...
    def _build_oneshot_card(self, parent):
        ...
        self._big_btn(box, "🕐  Schedule",
                      self._arm_oneshot, bg="#0e7490"
                      ).pack(fill="x", pady=(0, 4))
        self._big_btn(box, "✕  Disarm",
                      self._disarm_oneshot, bg=INPUT_BG
                      ).pack(fill="x")

    def _oneshot_status_text(self):
        os = self.rs.get_one_shot()
        if os.get("armed"):
            return f"⏳ Armed  {os.get('start_time','?')}  {os.get('duration','?')} min"
        return "Not armed"
    ...
    def _refresh(self):
        """Update all dynamic labels from scheduler + data_manager state."""
        sch = self.scheduler
        ...
        if today_rec:
            ...
            try:
                ...
                self._lbl_2nd_dur.config(text=f"{dur2_int} min" if dur2_int else "—")
                total = int(dur1) + dur2_int + manual_int
            except (ValueError, TypeError):
                total = 0
            total_text = f"{total} min"
            if manual_int:
                total_text += f"  (+{manual_int} manual)"
            self._lbl_total.config(
                text=total_text if total else "—",
                fg=ACC_ORANGE)
            ...
            try:
                total = int(cr.get('first_run', 0)) + int(dur2)
            except (ValueError, TypeError):
                total = 0
            self._lbl_total.config(
                text=f"{total} min (predicted)" if total else "—",
                fg=ACC_ORANGE)
        else:
        ...
        # ── Prediction (from cache or scheduler calc) ─────────
    ...
    def _apply_prediction(self, pred):
        self._lbl_pred_temp.config(
            text=f"{pred.get('avg_temp','?')} °C")
        self._lbl_pred_cloud.config(
            text=f"{pred.get('avg_cloud','?')} %")
        self._lbl_pred_eff.config(
            text=f"{pred.get('effective_temp','?')} °C")
        self._lbl_pred_dur.config(
            text=f"{pred.get('duration','?')} min",
            fg=ACC_BLUE)
        ts = datetime.now().strftime("%H:%M")
        self._pred_status_lbl.config(
            text=f"Last updated {ts}", fg=TEXT_DIM)

    def _refresh_status(self):
        """Query live boiler / HA / MQTT status in background."""
        def _work():
            # MQTT broker
            if self.mqtt_service:
                connected = self.mqtt_service.is_connected()
                self.parent.after(0, lambda c=connected:
                    self._set_mqtt_status(c))
                if connected:
                    s = self.mqtt_service.get_status() or "unknown"
                    self.parent.after(0, lambda st=s:
                        self._set_boiler_status(st))
            ...
            try:
                ...
                ha_ok = False
            self.parent.after(0, lambda ok=ha_ok: self._set_ha_status(ok))

            # HA boiler state (if HA mode)
            if self.rs.get_execution_mode() == "HA":
                entity = getattr(self.config, "boiler_entity_id", "") or                     self.config.config["homeassistant"].get("BOILER_ENTITY_ID", "")
                try:
    ...
    # ─────────────────────────────────────────────────────────
    # PUBLIC — called by main_window fan-out
    # ─────────────────────────────────────────────────────────
    ...
    def set_ha_status(self, ok: bool):
        self.parent.after(0, lambda: self._set_ha_status(ok))

    def set_mqtt_status(self, connected: bool):
        self.parent.after(0, lambda: self._set_mqtt_status(connected))

    # ─────────────────────────────────────────────────────────
    # STATUS PILL UPDATERS
    # ─────────────────────────────────────────────────────────

    def _set_boiler_status(self, status: str):
        s = str(status).upper()
        # Forward to status bar via callback — dots removed from dashboard
        if self.on_boiler_state:
            self.on_boiler_state(s)

    def _set_ha_status(self, ok: bool):
        pass   # Status bar handles this now

    def _set_mqtt_status(self, connected: bool):
        pass   # Status bar handles this now
    ...
    def _start_now(self):
        try:
            ...
            messagebox.showerror("Invalid", "Duration must be a positive integer")
            return
        ...
        def _run():
            ...
            color = ACC_GREEN if ok else ACC_RED
    ...
    def _disarm_oneshot(self):
        os = self.rs.get_one_shot()
        self.rs.set_one_shot(
            os.get("start_time", "20:00"),
            int(os.get("duration", 30)),
            armed=False)
